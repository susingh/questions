...
namespace Questions.IK
{
    ...
    class DP : IQuestion
    {
        ...
        int maxPath(int[,] arr, int i, int j)
        {
            // base condition
            int r = arr.GetLength(0);
            int c = arr.GetLength(1);
        ...
        int maxPathMemoization(int[,] arr, int[,] result, int i, int j)
        {
            // base condition
            int r = arr.GetLength(0);
            int c = arr.GetLength(1);
            ...
            {
                downMax = maxPathMemoization(arr, result, i, j + 1);
                result[i, j+1] = downMax;
            }

            int rightMax = int.MinValue;
            if (result[i + 1, j] != int.MinValue)
            {
                rightMax = result[i + 1, j]; 
            }
            else
            {
                rightMax = maxPathMemoization(arr, result, i + 1, j);
                result[i+1, j] = rightMax;
            }

            return arr[i, j] + Math.Max(rightMax, downMax);
        }

        int maxPathDP(int[,] arr)
        {
            int r = arr.GetLength(0);
            int c = arr.GetLength(1);

            int[,] dpTable = new int[r+1, c+1];
            ...
            dpTable[r - 1, c - 1] = arr[r - 1, c - 1];

            for (int i = r - 1; i >= 0; i--)
            {
                for (int j = c - 1; j >= 0; j--)
                {
                    //dpTable[i, j] = Math.Max arr[] 
                }
        ...
        int CoinChangeDP(int[] coins, int sum)
        {
            ...
            for (int change = 1; change <= sum; change++)
            {
                int minCoins = int.MaxValue;
                ...
                foreach (var coin in coins)
                {
                    if (coin <= change)
                    {
                        int temp = dpTable[change - coin];
                        if (temp == int.MaxValue) continue;
                        minCoins = Math.Min(minCoins, 1 + temp);
                    }
                }

                dpTable[change] = minCoins;
